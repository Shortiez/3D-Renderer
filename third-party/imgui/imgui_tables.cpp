...
static const float TABLE_BORDER_SIZE                     = 1.0f;    // FIXME-TABLE: Currently hard-coded because of clipping assumptions with outer borders graphics.
...
//   FIXME: But the graphics code in TableEndRow() nullifies that with clamping required for scrolling.
    ...
    if (is_visible)
    {
        ...
        if (draw_cell_bg_color)
        {
            ...
            for (ImGuiTableCellData* cell_data = &table->RowCellData[0]; cell_data <= cell_data_end; cell_data++)
            {
                ...
                // FIXME: This cancels the OuterPadding addition done by TableGetCellBgRect(), need to keep it while graphics correctly while scrolling.
...
void ImGui::TableMergeDrawChannels(ImGuiTable* table)
{
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
    {
        ...
        for (int merge_group_sub_n = 0; merge_group_sub_n < merge_group_sub_count; merge_group_sub_n++)
        {
            ...
            // (note that we assume that graphics didn't stray on the left direction. we should need a CursorMinPos to detect it)
